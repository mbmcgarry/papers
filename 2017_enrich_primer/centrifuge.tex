...
$r_{1}$ = radius at which waste is extracted\TODO{Not certain about this defn} \\
...
x = pressure ratio (for fast machines is $\sim$1000:1) \TODO{pressure ratio of what? r2 to r1?}
\\

Using energy balance of an ideal gas, Dirac defined a theoretical ideal separation potential $\delta U_{Dirac}$: \TODO{This is eqn 6  from Glaser SGS. How to cite properly? Is it plagarism to republish equations?}
...
\TODO{Is efficiency in the table defined as $delta U_{Raetz}/ \delta U_{Dirac}$?}

Given the equations above, as well as a feed assay ($N_{F}$), the enrichment factor ($\alpha $), product assay ($N_{P}$) and waste assay {$N_{W}$) are:

\begin{equation}
  \label{eqn:sep_fac}
\alpha = 1+ \sqrt{\frac{2\delta U (1 - \theta)}{\theta F}}
\end{equation}

\begin{equation}
  \label{eqn:product_assay}
  N_{P} = \frac{1}{\frac{1-N_{F}}{\alpha N_{F}} + 1}
  \end{equation}

\begin{equation}
\label{eqn:waste_assay}
N_{W} = \frac{
  \frac{N_{f}}{\alpha (1-N_{F})}}
       {1+
         \frac{N_{F}}{\alpha (1-N_{F})}}
\end{equation}

One important note for the reader: Equation \ref{eqn:raetz} is typically discussed in the literature as being in terms of molar fraction or moles of \gls{UF6} per second. Equation \ref{eqn:sep_fac}, and most of the equations in the cascade section, are in terms of atom fraction or kilograms of uranium per year. This is a discrepancy found both in the technical literature and in the non-proliferation policy world. For example, Iran often reports its \gls{SWU} capacity in terms of enriched kilograms of \gls{UF6}. In the west when Iran’s enrichment limits are in terms of kilograms of enriched uranium – because that’s the mass that counts against the limits in the \gls{JCPOA} and the 90kg necessary to build a nuclear weapon. Therefore, when comparing reported flow rates to theoretical flow rates, remember to convert between molecular mass and atomic mass:

\begin{equation}
  \label{eqn:convert}
  \delta U_{atom} = \delta U_{UF_{6}} \left ( \frac{m_{atom}}{m_{UF_6}} \right )
  \end{equation}
